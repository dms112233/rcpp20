...
public class pointCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String string, String[] strings) {
        if (strings[0].equalsIgnoreCase("capture") || strings[0].equals("")){
            String closestPoint = "";
            double closestX = 0;
            double closestY = 0;
            double closestZ = 0;
            ...
            if (!allPoints.isEmpty()) {
                ...
                commandSender.sendMessage(AQUA + langPrint("Nearest point: ","Ближайшая точка: ") + closestPoint);
                commandSender.sendMessage(AQUA + langPrint("Distance to a point: ","Расстояние до точки: ") + minDistance);
...
//                        }
                        ...
                        int CountInOwnerCommand = 2;
                        if (Rim4oosCapturePointsPlugin.getData().getTeamVl(closestPoint) != "null"){
                            CountInOwnerCommand = teamCounts.getOrDefault(Rim4oosCapturePointsPlugin.getData().getTeamVl(closestPoint), 0);
                        }

                        if (CountInSideCommand >= MinPLayersOnlineInCommand | CountInOwnerCommand >= 2 ){
                            if(!Rim4oosCapturePointsPlugin.getData().getTeamVl(closestPoint).equalsIgnoreCase(getTeamForPlayer(player))) {
                                if (Rim4oosCapturePointsPlugin.getData().getWork(closestPoint)) {
                                    commandSender.sendMessage(RED + langPrint("Failed to capture the point","Не удалось захватить точку"));
                                    commandSender.sendMessage(RED + langPrint("The point is still being launched by the enemy", "Точка ещё запущена врагом"));
                                }else if(CountInPlayerTeam < 2){
                                    commandSender.sendMessage(RED + langPrint("Failed to capture the point","Слишком мало игроков в вашей команде"));
                                }
                                    ...
                                    Bukkit.broadcastMessage(LIGHT_PURPLE + langPrint("Team ","Команда ") + getTeamForPlayer(player) + langPrint(" has captured point "," захватила точку ") + output);
                                ...
                                commandSender.sendMessage(RED + langPrint("The point is already yours","Точка уже ваша"));
                                if(Rim4oosCapturePointsPlugin.getData().getWork(closestPoint)){
                                    commandSender.sendMessage(RED + langPrint("The point's already working","Точка уже работает"));
                                }else{
                                    itemSpawner.stop();
                                    itemSpawner.start(time);
                                    commandSender.sendMessage(GREEN +"Point launched");
                                }
                            }
                        }else if(Rim4oosCapturePointsPlugin.getData().getTeamVl(closestPoint).equalsIgnoreCase(getTeamForPlayer(player))) {
                            commandSender.sendMessage(RED + langPrint("The point's already working","Точка уже ваша"));
                            if(Rim4oosCapturePointsPlugin.getData().getWork(closestPoint)){
                                commandSender.sendMessage(RED + langPrint("","Точка уже работает"));
                            }else{
                                itemSpawner.stop();
                                itemSpawner.start(time);
                                commandSender.sendMessage(GREEN +langPrint("Point launched","Точка запущена"));
                            }
                        }
                        else {
                            commandSender.sendMessage(RED + langPrint("Too few players on enemy teams","Слишком мало игроков во вражеских команд"));
                        }
                        ...
                        commandSender.sendMessage(RED + langPrint("Too few players on the server","Слишком мало игроков на сервере"));
                    }

                } else {
                    commandSender.sendMessage(RED + langPrint("Too far away!","Слишком далеко!"));
                }
            } else {
                commandSender.sendMessage(RED + langPrint("Points not found!","Точки не найдены!"));
            }
        ...
        if(strings[0].equalsIgnoreCase("list")) {
            ...
            for (String p: allPoints){
                ...
                commandSender.sendMessage(langPrint("Point ","Точка ") + AQUA + p + YELLOW + " " + Rim4oosCapturePointsPlugin.getData().getTeamVl(p)  + " \n" + WHITE + materiaGlList);
            }
        }
        ...
        if (strings[0].equalsIgnoreCase("clear")) {
            if (commandSender.hasPermission("rcpp.clearPoint")) {
                if (strings[1].equalsIgnoreCase("all")) {
                    ...
                    commandSender.sendMessage(AQUA + langPrint("All points were successfully removed.","Все точки были успешно удалены."));
                } else {
                    String key = strings[1];
                    if (Rim4oosCapturePointsPlugin.getData().getConfig().contains(key)) {
                        Rim4oosCapturePointsPlugin.getData().getConfig().set(key, null);
                        commandSender.sendMessage(AQUA + langPrint("Point ","Точка ") + key + langPrint(" has been successfully deleted."," была успешно удалена."));
                    } else {
                        commandSender.sendMessage(RED + langPrint("Point ","Точка ") + key + langPrint(" not found."," не найдена."));
                    }
                }
                Rim4oosCapturePointsPlugin.getData().save();
            } else {
                commandSender.sendMessage(RED + langPrint("Not enough rights!","Недостаточно прав!"));
            }
        }
        ...
        if (strings[0].equalsIgnoreCase("reload")) {
            ...
            if (commandSender.hasPermission("rcpp.reload")) {
                commandSender.sendMessage(AQUA + langPrint("Successful!","Успешно!"));
            } else {
                commandSender.sendMessage(RED + langPrint("Not enough rights!","Недостаточно прав!"));
            }
        }
        ...
        if (strings[0].equalsIgnoreCase("add")) {
            if (commandSender.hasPermission("rcpp.addPoint")) {
                ...
                if(!strings[4].equalsIgnoreCase("all")) {
                    ...
                    commandSender.sendMessage(RED + langPrint("Well, that was genius.","Ну это было гениально"));
                }
            }
            if (!commandSender.hasPermission("rcpp.addPoint")) {
                commandSender.sendMessage(RED + langPrint("Not enough rights!","Недостаточно прав!"));
